...
- Phase 2で作成済みの工程行0件注文は経過措置として、工程行がないことを完了拒否理由にしない。
- Phase 3で工程行が存在する注文は、蒸熱、揉捻、乾燥の全3工程が`COMPLETED`であることを必須とする。
- 製造指示と製品在庫行をlock取得後に再検証する。
- 製品加算、増減履歴、`COMPLETED`への変更を同一transactionで行う。

新しくissueする製造指示には固定3工程を重複なく作成する。工程は製造中だけsequence順に開始・完了し、開始・完了日時を保存する。歩留まり計算は行わないため、製造指示の予定数量を製品在庫へ加算する。
